...
.wc-back { color: #181818; font-size: 18px; font-weight: 500; }
.wc-room-name {
  color: #181818; font-size: 16px; font-weight: 500; flex: 1; text-align: center;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  min-width: 0;
}
.wc-menu { color: #181818; font-size: 18px; font-weight: 700; white-space: nowrap; letter-spacing: 1px; }
...
.time-divider { text-align: center; margin: 2px 0; }
.time-divider span {
  background: rgba(0,0,0,0.12); color: #fff;
  font-size: 11px; padding: 2px 8px; border-radius: 4px;
}

.section-divider { text-align: center; margin: 2px 0; }
...
.avatar-col { display: flex; flex-direction: column; align-items: center; width: 40px; flex-shrink: 0; }
.avatar {
  width: 40px;
  height: 40px;
  border-radius: 5px;
...
.sender-name { font-size: 12px; color: #9b9b9b; margin-bottom: 4px; padding-left: 2px; }
...
.bubble.left::before {
  left: -11px; border-right-color: var(--bubble-bg, #fff);
}
.bubble.right { background: #95ec69; }
.bubble.right::before {
  right: -11px; border-left-color: var(--bubble-bg, #95ec69);
}
...
.wc-input {
  ...
  border-radius: 5px; padding: 7px 14px; font-size: 14px; color: #181818;
  min-height: 34px;
}
.wc-extra {
  width: 30px; height: 30px; flex-shrink: 0; color: #54565a;
...
<div class="device">
  ...
  <div class="wc-footer">
    <div class="wc-voice">
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#54565a" stroke-width="1.6">
...
function appendMsg(d) {
  ...
  if (d.image) html += '<img class="bubble-img" src="' + d.image + '" onload="window.__imgLoaded__ && window.__imgLoaded__()" onerror="this.remove()" />';
